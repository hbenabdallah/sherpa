...
It runs on a model **online**, through any OpenAI-compatible API (OpenAI,
Mistral, Groq, OpenRouter, Cloudflare Workers AI, vLLM, LM Studio…), or **on your
machine** with Ollama. Written in PHP 8.4 and Symfony 8; MIT-licensed.
...
Docker, nothing else to install. Take the latest version from the
[Releases](../../releases) page:

```sh
version=0.1.2
base="https://github.com/hbenabdallah/sherpa/releases/download/v$version"
platform=$(uname -s | tr 'A-Z' 'a-z' | sed 's/darwin/macos/')-$(uname -m | sed 's/arm64/aarch64/')
file="sherpa-$version-$platform"
curl -fLO "$base/$file"
curl -fLO "$base/$file.sha256"
sha256sum -c "$file.sha256" 2>/dev/null || shasum -a 256 -c "$file.sha256"
mkdir -p ~/.local/bin && install -m 0755 "$file" ~/.local/bin/sherpa
```

`~/.local/bin` must be in your `PATH`. On macOS, a file downloaded with a
browser rather than `curl` is quarantined and refused at launch:
`xattr -d com.apple.quarantine ~/.local/bin/sherpa` lifts that.
...
| `/permissions`, `/mcp`, `/skills`, `/tools` | the rest |

At the prompt, `/` opens a list of the commands and skills, narrowed as you
...
tools, which always ask before running. See `mcp.json.dist`.
...
| `SHERPA_API_URL`, `SHERPA_API_MODEL` | override what `config.yaml` says |
...
make release VERSION=1.2.0    # the single executable, checked end to end
```

`make release` packs Sherpa into a phar appended to the static PHP `micro`
runtime, pinned by checksum, then runs it against a fake provider
(`tests/release/smoke.sh`). A tag `v1.2.0` does the same in CI and publishes the
executable, its SHA-256 and `THIRD-PARTY-NOTICES.md` in the Releases.
