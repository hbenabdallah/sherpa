...
One file for Linux x86_64 — no PHP, no Docker, nothing else to install. Take the
latest version from the [Releases](../../releases) page:
...
```sh
export SHERPA_API_KEY=…      # your provider's key; never written to disk
cd ~/code/my-project
sherpa
```
...
Options: `-b api|ollama` (backend for this run), `-m <model>`, `-c <tokens>`
...
A choice was kept when a measurement said so:
...
| Where | What |
|---|---|
| `~/.config/sherpa/config.yaml` | backend, provider address, chosen models, prices (to show a cost) |
...
| Variable | |
|---|---|
| `SHERPA_API_KEY` | the provider's key |
...
- The executable exists for Linux x86_64 only, for now.
