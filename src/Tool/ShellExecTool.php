...
class ShellExecTool
{
     ...
     * Floors, not limits.
     *
     * These were flat caps chosen against a 32k window: a hundred lines of a
     * failing test run, which is very often fewer lines than the failures
     * themselves occupy. Truncating there is not frugal, it is discarding the
     * answer — the model reads a report whose verdict was cut off and reruns the
     * suite to see it, which costs a whole extra turn to recover output that
     * would have fitted.
     *
     * They now scale with the window and keep these values as the floor, so a
     * small window behaves exactly as it always did.
     */
    ...
    private function trim(string $output): string
    {
        ...
        if (count($lines) > $lineCap) {
            ...
            $lines[] = '... (tronqué à ' . $lineCap . ' lignes)';
        }
