...
/**
 * What this project has been worked on lately, as a handful of keywords.
 *
 * MemoryStore::digest() had to rank by recency and said so: the system prompt
 * is built before anyone has asked anything, so there is no query to rank
 * against. True — and it quietly assumed that no query means no information.
 *
 * There is plenty. At the moment the prompt is built we know what git has
 * touched since the last session, and which directory the user was standing in
 * when they launched. A fact about src/Payment/ is worth quoting in full on the
 * morning somebody is working in src/Payment/, and worth one line in the key
 * index on every other morning. That is retrieval whose query is the situation
 * rather than a sentence, and it costs no inference at all.
 */
final class RecentActivity
{
    ...
    /**
     * Paths git has touched recently: uncommitted first, then recent commits.
     *
     * Silence on failure is deliberate and total. This is a ranking hint; a
     * project that is not a repository, or a git that is not installed, must
     * cost the digest its boost and nothing else.
     *
     * @return string[]
     */
