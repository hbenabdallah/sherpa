...
class Terminal
{
    ...
    // The logo's two colours (public/sherpa.jpeg), in the 256-colour palette
    // every current terminal has: the slate peak, and the lime one in front.
    const SLATE = "\e[38;5;109m";
    const LIME  = "\e[38;5;149m";
    ...
    /**
     * Strip everything that could move the cursor, repaint the screen or latch
     * a colour on: a stray "\e[A" in text from outside overwrites the line
     * above, which is how a reply lands on top of the prompt. Byte-wise is safe
     * for UTF-8 — every byte removed is below 0x80.
     */
    ...
    public function restoreSane(): void
    {
        ...
        // Quitting from inside the scroll view must not leave the shell on
        // the alternate screen, with the session's output out of sight.
        if (self::$altScreen) {
    ...
    /** Whether the alternate screen is up, for restoreSane(). Static: any instance may quit. */
    private static bool $altScreen = false;

    /** Draw on a screen of its own; leaving puts back the one underneath, untouched. */
    public function enterAltScreen(): void
    {
        self::$altScreen = true;
        echo "\e[?1049h\e[H\e[2J";
    }
