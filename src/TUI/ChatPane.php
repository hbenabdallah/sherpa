...
class ChatPane
{
    ...
    /** Whether the waiting line is on screen and must be erased before anything else. */
    private bool $waiting = false;
    private float $lastWaitFrame = 0.0;

    /** Redraws of the waiting line, at most — reasoning can arrive fifty fragments a second. */
    private const WAIT_FRAME_SECONDS = 0.1;
    ...
    public function beginAssistantMessage(): void
    {
        ...
        // Straight away rather than after the first quiet poll: the wait
        // starts now, and so does the question of whether anything is
        // happening.
        $this->showWaiting();
    }
    ...
    /**
     * An animated "working" line while nothing else is on screen.
     *
     * Before this, a turn was silent until its first word: a second on a
     * hosted API, minutes on a laptop CPU, and however long a reasoning model
    ...
    public function showSpinner(string $message = 'Sherpa is thinking...'): void
    {
    ...
    private function renderLast(): void
    {
        ...
        if ($msg['role'] === 'user') {
            echo "\n" . Terminal::BOLD . Terminal::GREEN . 'You ' . Terminal::RESET
