...
class MemoryStore
{
    ...
    /**
     * How the digest ranks, now that it has more than recency to go on.
     *
     * Situation leads because it is the only signal that knows what today is
     * about: a fact naming src/Payment/ earns its full quotation on the morning
     * someone is working in src/Payment/, and a line in the key index on every
     * other morning. Usage is the long-run signal — a fact the model keeps
     * coming back for has proven it belongs in the prompt. Recency is what is
     * left when neither has anything to say, which is where this started.
     */
    ...
    public function digest(int $maxChars = 1600, array $terms = []): string
    {
        ...
        // Recency used to be the only ranking available, on the grounds that
        // the system prompt is built before anyone has asked anything. No
        // question, no query — but "no query" was mistaken for "no
        // information". What git has touched and where the user launched are
        // both known at this moment, and both cost nothing to consult.
    ...
    /**
     * Record that these facts were looked up on purpose.
     *
     * Deliberately not called by recall() itself: digest() goes through recall()
     * to build the prompt, and counting that would mean every fact is "used"
     * every session, which measures nothing. Only an explicit memory_recall
     * counts — that is a fact the model went looking for.
     *
    ...
    /**
     * Tally one search, whether or not it found anything.
     *
     * This is the instrument that decides a question left open: whether the
     * keyword search is good enough. A recall that comes back empty on a
     * project that has facts is a vocabulary mismatch — the model asked for
     * "connexion utilisateur" and the fact is filed under auth_strategy. If
     * that number stays low, BM25 was sufficient and nothing more is needed. If
     * it climbs, it is the evidence for embeddings, measured rather than
     * assumed.
     */
    public function noteRecall(bool $found): void
    {
        ...
        $this->conn->executeStatement(
            'INSERT INTO memory_stats (key, value) VALUES (:key, 1)
             ON CONFLICT(key) DO UPDATE SET value = value + 1',
            ['key' => $found ? 'recall_hits' : 'recall_misses'],
        );
    }
    ...
    public function recallStats(): array
    {
        ...
        $rows = $this->conn->fetchAllKeyValue('SELECT key, value FROM memory_stats');

        return [
            'hits'   => (int) ($rows['recall_hits'] ?? 0),
            'misses' => (int) ($rows['recall_misses'] ?? 0),
        ];
    }
    ...
    private function migrate(): void
    {
        ...
        $this->conn->executeStatement(
            'CREATE TABLE IF NOT EXISTS memory_stats (key TEXT PRIMARY KEY, value INTEGER NOT NULL DEFAULT 0)',
        );
     ...
     *
     * SQLite has no IF NOT EXISTS for columns, and a database that has been
     * through this migration once must not fail the second time — open() runs
     * on every session.
     */
