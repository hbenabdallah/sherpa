...
/**
 * What compaction took out of the window, still reachable.
 *
 * HistoryCompactor's cheap stage replaces an aged-out tool result with a stub
 * saying how big it was. That was a straight loss: the only way back to the
 * content was for the model to run the tool again — another turn, another full
 * prompt evaluation, to recover text Sherpa had already read and thrown away.
 *
 * So elision stops being a delete and becomes a swap. The body goes here on its
 * way out of the window, indexed, and context_recall pages it back in. The
 * corpus is the session's own tool output: small, hot, and the one body of text
 * whose relevance is not in question, because the agent just read it.
 *
 * In memory rather than on disk, and that is the whole lifetime argument: this
 * is the content of one conversation. A file read three days ago describes a
 * file that has since changed, and a stale excerpt is worse than none. The
 * store dies with the process, and /reset empties it early — the conversation
...
final class ContextStore
{
    ...
    /**
     * Lines one excerpt may contribute before it stops being an excerpt.
     *
     * A grep result where every line matches is the whole file, and returning
     * the whole file is what this store exists to avoid. The overflow is
     * counted rather than dropped in silence: knowing that 160 more lines
     * matched is what tells the model to ask a narrower question.
     */
    ...
    public function get(int $id, int $maxChars): ?string
    {
        ...
        $header = "[#{$id} · {$row['tool']} · {$row['lines']} lignes]";

        if (mb_strlen($content) > $maxChars) {
            $content = mb_substr($content, 0, $maxChars)
                . "\n… (tronqué ; affinez avec context_recall(search) pour cibler un passage)";
        }
    ...
    /**
     * Passages matching a query, with their surrounding lines.
     *
     * Returning whole excerpts would defeat the point. What comes back is what
     * a person would copy out of a long file: the matching lines, numbered, with
     * enough either side to read them.
     *
        ...
        foreach ($rows as $row) {
            ...
            if ($spent + mb_strlen($block) > $maxChars) {
                ...
                $block = mb_substr($block, 0, max(0, $maxChars - 60))
                    . "\n… (coupé au budget ; affinez la recherche)";
            }
    ...
    private function excerptFrom(string $content, array $words): string
    {
        ...
        if ($dropped > 0) {
            $out[] = "   … {$dropped} autre(s) ligne(s) correspondent ; affinez la recherche.";
        }
