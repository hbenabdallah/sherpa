...
final class SlashCommands
{
    ...
    /**
     * Every command, for the menu under "/". A hint in <angle brackets> is
     * required, one in [square brackets] optional — which decides whether
     * Enter runs the command at once or waits for what follows it.
     *
     * @return list<array{name: string, hint: string, description: string}>
     */
    public static function catalogue(): array
    {
        $entries = [
            ...
            ['/provider', '[add|<name>|key|remove]', 'API providers: list, add, switch'],
            ['/backend', '[api|ollama]', 'Online or local model'],
            ['/context', '', 'What fills the context'],
            ['/compact', '', 'Compact the conversation now'],
            ['/reset', '', 'Start an empty conversation'],
            ['/sessions', '', "This project's saved conversations"],
            ['/resume', '[n]', 'Pick a conversation back up'],
            ['/memory', '', 'The facts remembered'],
            ['/remember', '<key> <value>', 'Remember a fact by hand'],
            ['/forget', '<key>', 'Drop a fact'],
            ['/docs', '[reindex|eval|<question>]', 'The documentation index'],
            ['/project', '[edit|forget]', 'The current project'],
            ['/permissions', '[revoke <tool>|clear]', "The project's standing grants"],
            ['/skills', '', 'The available skills'],
            ['/mcp', '', 'MCP servers, their resources and prompts'],
            ['/tools', '', 'The available tools'],
            ['/exit', '', 'Leave Sherpa'],
        ];

        return array_map(fn(array $e) => ['name' => $e[0], 'hint' => $e[1], 'description' => $e[2]], $entries);
    }
  ...
  Commands:
    ...
    /provider           API providers (/provider add, /provider <name>, /provider key <name>, /provider remove <name>)
    /skills             List the available skills (each also runs as /<skill> [request])
    /mcp                The MCP servers, their resources and prompts (each prompt runs as /<server>:<prompt>)
...
HELP . Terminal::RESET;
            ...
            case '/provider':
                $this->model->handleProvider(trim($args), $bag, $project);
                return true;
                ...
                if ($prompts !== []) {
                    ...
                    foreach ($prompts as $prompt) {
                        echo '  ' . Terminal::YELLOW . Terminal::plain($prompt['command'], singleLine: true) . Terminal::RESET
                            . ' ' . \App\Mcp\PromptArguments::usage($prompt['arguments'])
