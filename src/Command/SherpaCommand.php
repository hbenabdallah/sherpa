...
class SherpaCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // What "/" lists: the commands, then the skills and the MCP servers'
        // prompts, which run as commands too. Read at each keystroke, so a
        // skill written mid-session is there.
        $this->lineEditor->useCompletions(fn(): array => [
            ...
            ...array_map(
                fn(array $prompt) => [
                    'name'        => $prompt['command'],
                    'hint'        => PromptArguments::usage($prompt['arguments']),
                    'description' => 'MCP prompt · ' . $prompt['description'],
                ],
                $this->mcp->promptCommands(),
            ),
        ]);
        ...
        while (true) {
            ...
            // An MCP prompt: the server fills it in, and that is the message.
            if (str_starts_with($userInput, '/')) {
                try {
                    $promptMessage = $this->mcp->expandPrompt($userInput);
                } catch (McpException $e) {
                    $this->chat->showError($e->getMessage());
                    continue;
                }

                if ($promptMessage !== null) {
                    if ($promptMessage === '') {
                        $this->chat->showError('The server returned an empty prompt.');
                        continue;
                    }
