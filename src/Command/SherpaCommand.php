...
class SherpaCommand extends Command
{
        ...
        while (true) {
            ...
            try {
                ...
                if ($result->interrupted) {
                    ...
                    $this->chat->showError(
                        'Le modèle répétait le même appel d\'outil sans avancer ; tour arrêté. '
                        . 'Reformulez, ou donnez-lui le renseignement qui lui manque.'
                    );
    ...
    private function applyModel(ModelProfile $profile): void
    {
        ...
        // Three copies of the same fact, set together for the same reason: a
        // throughput learned from another model, like a tokenizer ratio learned
        // from another model, describes something that is no longer running.
        $this->speed->useModel($profile->model);
    }

    /**
     * Say so, once, when the graphics card is not actually being used.
     *
     * The window requested occupies VRAM on top of the weights, and when it no
     * longer fits Ollama spills the remainder onto the CPU rather than refusing
     * — at which point everything still works and everything is ten to a
     * hundred times slower, with nothing anywhere saying why. The README used
     * to hand this to the user as a thing to watch with `ollama ps`. It is a
     * measurement, and it takes one request.
     */
    private function reportResidency(): void
    {
        ...
        if ($residency->isCpuOnly()) {
            ...
            $this->chat->showInfo(
                'Le modèle tourne entièrement sur le processeur. '
                . 'Normal sans carte graphique ; sinon, il n\'a pas pu être chargé dessus.'
            );
        ...
        $this->chat->showInfo(sprintf(
            'Le modèle déborde sur le processeur (%s). La fenêtre demandée (%s tokens) occupe '
            . 'de la VRAM en plus des poids : réduisez-la avec /model pour que tout tienne sur la carte.',
            $residency->describe(),
...
HELP . Terminal::RESET;
            ...
            case '/memory':
                ...
                // The measurement that decides whether keyword search is
                // enough: a recall that comes back empty on a project that has
                // facts is the model asking in words nobody filed them under.
                $stats = $this->memoryStore->recallStats();
                $searches = $stats['hits'] + $stats['misses'];
                if ($searches > 0) {
                    printf(
                        "\n  %d recherche(s) mémoire · %d sans résultat (%d %%)\n",
                        $searches,
                        $stats['misses'],
                ...
                if ($kept > 0) {
                    echo Terminal::GRAY . "  {$kept} sortie(s) d'outil retirée(s) du contexte, "
                        . "toujours consultables par context_recall.\n" . Terminal::RESET;
                }
                return true;
            ...
            case '/context':
                ...
                printf("  %-22s %d tokens\n", 'cible de compaction', $this->budget->compactTarget());
                ...
                // Measured, never configured. Everything that used to assume a
                // machine now asks these two numbers instead.
                echo Terminal::BOLD . "\nMachine:\n" . Terminal::RESET;
                printf(
                    "  %-22s %s\n",
                    'prefill',
                ...
                $residency = $this->platform->residency();
                printf("  %-22s %s\n", 'résidence du modèle', $residency?->describe() ?? '(inconnue)');

                $reprefill = $this->speed->secondsToPrefill($estimated);
                if ($reprefill !== null) {
                    printf("  %-22s ~%.1f s\n", 'relecture du contexte', $reprefill);
                }
                return true;
