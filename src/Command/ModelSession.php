 ...
 * changed from /model or /backend. Apart from the command because a model
...
final class ModelSession
{
    ...
    public function apply(ModelProfile $profile): void
    {
        ...
        $identity = $profile->backend->value . ':' . $profile->model;
    ...
    public function configureApiEndpoint(): bool
    {
        ...
        // about again. Kept anyway on request — usually an unexported key.
        while (true) {
        ...
        // The key is never asked for: typed here it would sit in a terminal's
        // scrollback, written down it would sit in a file. It comes from the
        // environment, and saying so now saves a failed first request.
        if (!$this->platform->apiHasKey()) {
            echo Terminal::GRAY
                . "  No key (SHERPA_API_KEY). If this provider wants one, export it and start again:\n"
                . "    export SHERPA_API_KEY=…\n" . Terminal::RESET;
        }

        return true;
    }
    /** One typed address, asked until it is one; null to run locally instead. */
    private function askApiEndpoint(): ?string
    {
        while (true) {
            ...
            echo Terminal::YELLOW . "  \"{$typed}\" is not an http(s) address. Try again, or Enter to run locally.\n" . Terminal::RESET;
        }
    }
    /**
