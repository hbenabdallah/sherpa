...
final class OpenAiCompatiblePlatform implements PlatformInterface, EmbeddingBackend
{
    ...
    private function attempt(array $payload, ?callable $onToken, ?callable $onWait, int $negotiations): array
    {
        ...
        return $this->consume($response, $onToken, $onWait, $startedAt);
    }
     ...
     * @param array<string, mixed> $payload
     */
    private function send(array $payload, string $path = '/chat/completions', string $accept = 'text/event-stream'): \Symfony\Contracts\HttpClient\ResponseInterface
    {
        if ($this->baseUrl === '') {
    ...
    private function streamAnthropic(array $messages, array $tools, ?callable $onToken, ?callable $onWait): array
    {
        ...
        for ($attempt = 1; ; $attempt++) {
            ...
            $startedAt = microtime(true);
            $response = $this->send($payload, '/messages');
            $status = $this->statusOf($response);
            ...
            // Thinking blocks bound to a history that has since changed — a
            // session resumed after a compaction, say. Sent again without
            // them, once: the reply is then made without that reasoning.
        ...
        $stream = new AnthropicStream($onToken, $onWait);
