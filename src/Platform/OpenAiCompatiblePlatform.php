...
final class OpenAiCompatiblePlatform implements PlatformInterface, EmbeddingBackend
{
    ...
    private function explain(int $status, string $body, ?string $model = null): string
    {
        ...
        $prefix = match (true) {
            ...
            $status === 0   => $this->name() . ' injoignable',
    ...
    private function consume(
        ...
        if (!$cancelled) {
            $this->consumeLine($buffer, $content, $toolCalls, $onToken, $firstTokenAt);
     ...
     * the call it answers — some providers reject `name` outright.
     *
    ...
    public function name(): string
    {
        ...
        return $this->baseUrl === '' ? 'l\'API' : (parse_url($this->baseUrl, PHP_URL_HOST) ?: $this->baseUrl);
    }
