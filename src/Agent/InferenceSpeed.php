 ...
 * A dozen constants elsewhere used to encode the machine Sherpa was written on
 * — iterations per turn, shell output kept, whether summarising is affordable —
 * each one a guess about speed frozen as a number.
 *
 * The measurement was there all along: every reply closes with durations beside
 * the token counts, and two divisions turn them into tokens per second, free,
 * on every turn. The speed half of what ContextBudget does for size, so a
 * machine without a GPU is the slow end of a scale, not a special case.
 */
