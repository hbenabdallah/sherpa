...
class AgentLoop
{
    ...
    private function runTurn(MessageBag $bag): Result
    {
        ...
        for ($i = 0; $i < self::MAX_ITERATIONS; $i++) {
            ...
            // reasoning model can send back no text and no call. Asked again
            ...
            if ($repeats >= self::REPEATED_CALL_LIMIT) {
                ...
                // Already on screen; written again as a plain reply so the
                // history ends on it, as it would after any answer, and the
                // next turn does not read the question as still open.
                if ($saidWhileRepeating !== '') {
                    $bag->assistant($saidWhileRepeating);

                    return new Result($saidWhileRepeating, iterations: $i + 1, repetitionDetected: true, repliedBeforeStop: true);
                }
