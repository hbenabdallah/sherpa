...
final class ContextBudget
{
    ...
    public function __construct(
        ...
        // Where compaction stops, as opposed to where it starts. These used to
        // be the same number, which meant every compaction freed just enough to
        // fall back under the trigger and the next turn crossed it again. Each
        // of those crossings rewrites history from the front and costs a full
        // re-read of the window on the next request — so the cheap-looking
        // policy was the one paying the largest recurring bill in the system.
    ...
    /**
     * A character allowance sized as a share of the window rather than fixed.
     *
     * SystemPromptBuilder worked this out first, for the block of remembered
     * facts, and the reasoning generalises to every other cap in the codebase:
     * a constant that was chosen as "about this much of the window" stops being
     * that the moment the window changes, and every one of them was chosen
     * against a 32k window on a machine with no graphics card.
     *
     * The floor is what keeps that machine behaving exactly as it did. The
     * ceiling is there because none of these allowances should grow without
     * bound just because the window can.
     */
