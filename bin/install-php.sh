...
# Already installed from this very archive: nothing to do. The stamp is the
# pinned sum, so a new pin in the Makefile installs again.
if [ -x "$dir/php" ] && [ "$(cat "$dir/php.sha256" 2>/dev/null)" = "$expected" ]; then
    echo "✓ PHP $("$dir/php" -r 'echo PHP_VERSION;') already installed in $dir/php"
    exit 0
fi
...
# failed build.
curl -fsSL --max-time 300 --retry 5 --retry-delay 10 --retry-all-errors -o "$tmp/php.tgz" "$url"
