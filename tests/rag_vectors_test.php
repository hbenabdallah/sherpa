...
$failing = new class implements Retriever {
    ...
    public function retrieve(string $question, int $limit): array { throw new RuntimeException('injoignable'); }
};
$index = new DocIndex();
$index->open(DocSearch::databaseFor($working->project()));
$fallback = new FallbackRetriever($failing, new Bm25Retriever($index));
check('a strategy that throws hands over to the other', $fallback->retrieve('factures', 3) !== [] && $fallback->lastError() === 'injoignable');
