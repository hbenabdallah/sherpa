<?php
// Project identity. A project is its directory — one per directory — and its
// slug names the memory directory, so getting either wrong is not cosmetic.
...
// One project per directory: every project below gets a directory of its own.
$fresh = function () use ($work): string {
    $dir = $work . '/p-' . bin2hex(random_bytes(4));
    mkdir($dir, 0777, true);
...
$project = $store->create("Éditeur d'essai", $fresh(), $none);
check('an accented name keeps its letters', $project->slug === 'editeur-d-essai', $project->slug);

check('German sharp s is spelled out', $store->create('Straße', $fresh(), $none)->slug === 'strasse');
check('a Cyrillic name is transliterated', $store->create('Проект', $fresh(), $none)->slug === 'proekt');
// The same in both scripts glibc used to handle, whichever C library is
// underneath: musl — the static PHP of a host install — spells them "??????".
check('a Greek one too, accents and all', $store->create('Ελλάδα', $fresh(), $none)->slug === 'ellada');
check('and Ukrainian letters Russian does not have', $store->create('Київ', $fresh(), $none)->slug === 'kiyiv');
...
$symbols = $store->create('***', $fresh(), $none);
...
check('the same unspellable name derives the same slug', $store->create('***', $fresh(), $none)->slug === $symbols->slug);

// ---- collisions ------------------------------------------------------------
$first = $store->create('Duplicata', $fresh(), $none);
$second = $store->create('Duplicata', $fresh(), $none);
...
check('accented and unaccented names stay distinct',
    $store->create('Café', $fresh(), $none)->slug !== $store->create('Cafe', $fresh(), $none)->slug);
...
// that was deleted.
$doomed = $store->create('À supprimer', $fresh(), $none);
...
$again = $store->create('À supprimer', $fresh(), $none);
...
$before = $store->create('Avant', $fresh(), $none);
...
$moved = $store->update($after, 'Encore ailleurs', $fresh(), $none);
...
$alpha = $sessionA->create('Alpha', $fresh(), $none);
$sessionB2 = new ProjectStore(new StackDetector());   // B starts after alpha exists
$beta = $sessionB2->create('Beta', $fresh(), $none);
...
$gammaDir = $fresh();
$gamma = $sessionB2->create('Gamma', $gammaDir, $none);
$yaml = $home . '/.config/sherpa/projects.yaml';
file_put_contents($yaml, str_replace($gammaDir, $gammaDir . '/deplace', file_get_contents($yaml)));
mkdir($gammaDir . '/deplace', 0777, true);
...
check('a hand edit to an untouched project survives a later write',
    $afterEdit->get($gamma->slug)?->path === $gammaDir . '/deplace',
...
$other->create('Surprise', $fresh(), $none);
...
$watcher2->create('Nous', $fresh(), $none);
...
// ---- a directory becomes a project at its first message --------------------
// Opening Sherpa somewhere is not deciding to work there. Until the first
// message, the project exists in the session only.
$yaml = $home . '/.config/sherpa/projects.yaml';
$before = md5_file($yaml);
$newDir = $fresh();
$draft = $store->draft($newDir, 'Brouillon', $none);

check('a draft writes nothing to projects.yaml', md5_file($yaml) === $before);
check('nor creates its memory directory', !is_dir(dirname($draft->memoryDb)), dirname($draft->memoryDb));
check('and is not found by its directory yet', $store->forDirectory($newDir) === null);

$adopted = $store->adopt($draft);
check('adopted, it is saved', (new ProjectStore(new StackDetector()))->forDirectory($newDir)?->slug === $adopted->slug);
check('with its memory directory', is_dir(dirname($adopted->memoryDb)));

// One project per directory, whatever else happens.
$twin = $store->adopt($store->draft($newDir, 'Jumeau', $none));
check('a second draft for the same directory is the same project', $twin->slug === $adopted->slug, "{$twin->slug} vs {$adopted->slug}");
check('and adds no entry', count(array_filter($store->all(), fn($p) => $p->path === $adopted->path)) === 1);

symlink($newDir, $home . '/raccourci');
check('reached through a symlink, it is still the same directory',
    $store->forDirectory($home . '/raccourci')?->slug === $adopted->slug,
    (string) $store->forDirectory($home . '/raccourci')?->slug);
$viaLink = $store->adopt($store->draft($home . '/raccourci', 'Par le lien', $none));
check('and adopting it through the link finds the project instead of making a twin', $viaLink->slug === $adopted->slug);

// Two terminals opened side by side on a new directory: both hold a draft,
// the first to send a message wins, the second joins it.
$otherDir = $fresh();
$left = new ProjectStore(new StackDetector());
$right = new ProjectStore(new StackDetector());
$leftDraft = $left->draft($otherDir, 'Gauche', $none);
$rightDraft = $right->draft($otherDir, 'Droite', $none);
$leftProject = $left->adopt($leftDraft);
$rightProject = $right->adopt($rightDraft);
