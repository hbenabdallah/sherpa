...
$store->remember('auth', 'Authentication uses JWT tokens signed with RS256');
$store->remember('deployment', 'Deployment goes through Ansible to two servers');
...
$hits = $store->recall('deployment servers');
...
for ($i = 0; $i < 60; $i++) {
    $big->remember("fait_{$i}", "A project convention described in enough detail to be useful, number {$i}.");
}

$digest = $big->digest(1600);
check('the digest respects its character budget', mb_strlen($digest) <= 1600, (string) mb_strlen($digest));
check('it details the most recent facts in full', str_contains($digest, 'fait_59: A project convention'), $digest);
...
check('an empty memory says so rather than showing an empty section',
    str_contains($empty->digest(), 'nothing remembered'), $empty->digest());
...
{
    $bag = new MessageBag();
    $bag->system('SYSTEM: the project memory digest lives here');
    for ($i = 0; $i < $exchanges; $i++) {
        $bag->user("question {$i} sur l'architecture");
        $bag->assistant("reply {$i}");
    }
...
$reply = "Here are the facts I found (see [the code] and [the config]):\n"
    . "```json\n" . '[{"key":"auth_strategy","value":"JWT via lexik"},{"key":"tests","value":"make test [not phpunit]"}]' . "\n```\nJ'espère que cela aide.";
...
$result = extractor($fresh, 'I cannot answer that request.')->extract(conversation());
check('a reply with no JSON at all is reported as a failure', $result->error !== null, $result->summary());
check('and the failure is legible', str_contains($result->summary(), 'Cannot extract'), $result->summary());
...
$sloppy = '[{"key":"ok","value":"a proper value"},{"key":"orphan"},{"value":"no key"},'
    . '{"key":"","value":"empty key"},{"key":"trop_long","value":"' . str_repeat('x', 500) . '"},'
    . '{"key":["an","array"],"value":"non-scalar key"}]';
...
// to mean there was no query. There is no *sentence*; there is plenty of
// information — what git touched, where the user launched.

$rankDir = sys_get_temp_dir() . '/sherpa-rank-' . bin2hex(random_bytes(4));
$ranked = new MemoryStore();
$ranked->open($rankDir . '/memory.db');

$ranked->remember('payment_gateway', 'Stripe via un service PaymentGateway dans src/Payment');
usleep(1100);
$ranked->remember('mailer_transport', 'Symfony Mailer avec un transport SMTP maison');
usleep(1100);
$ranked->remember('deploy_procedure', 'make deploy pousse sur le serveur de prod');

// Recency alone puts deploy_procedure first — it was written last.
$plain = $ranked->digest(4000);
check('with no situation, the most recent fact leads',
    strpos($plain, 'deploy_procedure') < strpos($plain, 'payment_gateway'), $plain);

// Someone working in src/Payment wants the payment fact quoted, on a morning
// when nothing about it has changed in days.
$situated = $ranked->digest(4000, ['payment', 'stripe']);
check('a fact about what is being worked on leads instead',
    strpos($situated, 'payment_gateway') < strpos($situated, 'deploy_procedure'), $situated);
check('and the others are still there, just after it',
    str_contains($situated, 'mailer_transport'), $situated);

// Terms that match nothing must not reorder anything.
$unrelated = $ranked->digest(4000, ['kubernetes', 'terraform']);
check('terms that match nothing leave the order alone', $unrelated === $plain, $unrelated);

// ---- a fact that gets looked up earns its place ----------------------------
$stats = $ranked->recallStats();
check('nothing is tallied before any search', $stats['hits'] === 0 && $stats['misses'] === 0, json_encode($stats));

$ranked->noteRecall(true);
$ranked->noteRecall(false);
$ranked->noteRecall(false);
$stats = $ranked->recallStats();
check('searches are tallied hit and miss', $stats['hits'] === 1 && $stats['misses'] === 2, json_encode($stats));

$mailer = null;
foreach ($ranked->recall() as $fact) {
    if ($fact->key === 'mailer_transport') {
        $mailer = $fact;
    }
}
check('a fact starts with no recalls to its name', $mailer !== null && $mailer->recalledCount === 0);

// Looked up five times: the model keeps coming back for it.
for ($i = 0; $i < 5; $i++) {
    $ranked->markRecalled([$mailer->id]);
}
...
check('recalls are counted', $used !== null && $used->recalledCount === 5, (string) $used?->recalledCount);
check('and dated', $used?->lastRecalledAt !== null);

check('a fact the model keeps coming back for climbs the digest',
    strpos($ranked->digest(4000), 'mailer_transport') < strpos($ranked->digest(4000), 'deploy_procedure'),
    $ranked->digest(4000));

// digest() reaches facts through recall(); if that counted as usage, every
// fact would be "used" every session and the signal would measure nothing.
$ranked->digest(4000);
$after = null;
foreach ($ranked->recall() as $fact) {
    if ($fact->key === 'payment_gateway') {
        $after = $fact;
    }
}
check('building the prompt does not count as using a fact', $after !== null && $after->recalledCount === 0,
    (string) $after?->recalledCount);

check('marking nothing is harmless', (function () use ($ranked) {
    $ranked->markRecalled([]);
...
$v2Dir = sys_get_temp_dir() . '/sherpa-v2-' . bin2hex(random_bytes(4));
mkdir($v2Dir, 0777, true);
$v2 = $v2Dir . '/memory.db';
$pdo = new PDO('sqlite:' . $v2);
$pdo->exec('CREATE TABLE facts (id INTEGER PRIMARY KEY AUTOINCREMENT, key TEXT NOT NULL, value TEXT NOT NULL,
            created_at TEXT NOT NULL, updated_at TEXT NOT NULL, superseded_at TEXT NULL, UNIQUE(key, value))');
$pdo->exec("INSERT INTO facts (key, value, created_at, updated_at)
            VALUES ('orm', 'Doctrine ORM 3', '2026-01-01T00:00:00+00:00', '2026-01-01T00:00:00+00:00')");
$pdo->exec('PRAGMA user_version = 2');
$pdo = null;

$upgraded = new MemoryStore();
$upgraded->open($v2);
$facts = $upgraded->recall();
check('a v2 database keeps its facts through the v3 migration',
    count($facts) === 1 && $facts[0]->value === 'Doctrine ORM 3', json_encode(values($facts), JSON_UNESCAPED_UNICODE));
check('and its facts simply start at zero recalls', $facts[0]->recalledCount === 0);
$upgraded->markRecalled([$facts[0]->id]);
check('the new columns are usable straight away', $upgraded->recall()[0]->recalledCount === 1);

// open() runs every session; the second one must not try to add them again.
$reopened = new MemoryStore();
$reopened->open($v2);
check('re-opening a v3 database is a no-op', $reopened->recall()[0]->recalledCount === 1);

exec('rm -rf ' . escapeshellarg($dir) . ' ' . escapeshellarg($legacyDir)
    . ' ' . escapeshellarg($rankDir) . ' ' . escapeshellarg($v2Dir));
printf("\n%d passed, %d failed\n", $pass, $fail);
exit($fail === 0 ? 0 : 1);
