...
# The release number is in the file name: sherpa-<version>-<os>-<arch>.
expected=$(basename "$1" | sed -E 's/^sherpa-(.*)-(linux|macos)-[a-z0-9_]+$/\1/')
...
# Colours in between would split what is looked for. The escape character is
# spelled out: macOS sed does not read \x1b.
esc=$(printf '\033')
