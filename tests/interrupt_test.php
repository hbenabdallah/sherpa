...
$chunks = [
    ...
    '{"message":{"content":"in full"},"done":true}',
];

$quiet = new Interrupt();
$platform = new OllamaPlatform(new MockHttpClient(new MockResponse($chunks)), 'http://x', 'm', 32768, -1.0, $quiet);
check('without cancellation the whole reply arrives', $platform->stream([])['content'] === 'a reply in full');
...
final class TripTool
{
    ...
    public function __invoke(#[Param('unused')] string $note = ''): string
    {
...
check(
    ...
    str_contains(json_encode($messages, JSON_UNESCAPED_UNICODE), 'Interrupted by the user'),
