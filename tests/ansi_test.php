...
/**
 * What is left on screen once the waiting line has erased itself: it is drawn
 * with \r and wiped the same way, so it never survives into the scrollback.
 */
function settled(string $out): string
{
    return (string) preg_replace('/\r[^\r]*\r {60}\r/u', '', $out);
}
...
// ---- the waiting line ------------------------------------------------------
// A turn used to be silent until its first word — a second online, minutes on
// a laptop CPU, however long a reasoning model thought. The line says work is
// happening, and must never reach the scrollback.
$clear = "\r" . str_repeat(' ', 60) . "\r";

$pane = new ChatPane(new Terminal(), new MarkdownRenderer());
$opened = captured(fn() => $pane->beginAssistantMessage());
check('a turn opens on a waiting line', str_contains($opened, 'Sherpa is thinking'), json_encode($opened));

$thinking = captured(function () use ($pane) {
    usleep(110_000);
    $pane->showWaiting(true);
});
check('a reasoning model is said to be reasoning', str_contains($thinking, 'Sherpa is reasoning'), json_encode($thinking));

$first = captured(function () use ($pane) {
    $pane->appendToken('Bonjour');
    $pane->flushBuffer();
});
check('the first output erases it before printing', str_starts_with($first, $clear), json_encode($first));
check('once, not before every line after', !str_contains(captured(fn() => $pane->showInfo('suite')), $clear));

$toolOnly = new ChatPane(new Terminal(), new MarkdownRenderer());
captured(fn() => $toolOnly->beginAssistantMessage());
check('a turn that only calls a tool clears it too',
    str_starts_with(captured(fn() => $toolOnly->showToolCall('file_read', ['path' => 'a.php'])), $clear));
...
check('a tool-only turn prints no orphan label', !str_contains(settled($out), 'Sherpa'), bin2hex($out));
...
check('and it leaves no orphan label behind', !str_contains(settled($out), 'Sherpa'), bin2hex($out));
...
check('a tool_call tag split across tokens is withheld', trim(settled($out)) === '', bin2hex($out));
