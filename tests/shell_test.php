...
check('a silent success says so', str_contains($tool('true'), 'no output'), $tool('true'));
...
check('a failing command with output is marked as failing', str_contains($failed, 'exit code 1'), $failed);
check('and its output is still there', str_contains($failed, '2 failures'), $failed);
check('a succeeding command carries no exit-code noise', !str_contains($tool('echo ok'), 'exit code'), $tool('echo ok'));

// ---- output has to fit in a context window ---------------------------------
$many = $tool('seq 1 500');
check('long output is cut to 100 lines', substr_count($many, "\n") <= 101, (string) substr_count($many, "\n"));
check('and says that it was cut', str_contains($many, 'truncated to 100 lines'), substr($many, -120));
...
check('and says that too', str_contains($huge, 'truncated to'), substr($huge, -120));
...
check('and said to have been stopped', str_contains($result, 'interrupted'), $result);
...
$dockerised = new ShellExecTool($paths, $app);
$dockerised->setDockerContainer('monapp-app-1');
...
check('the container is named', str_contains($described, 'monapp-app-1'), $described);
...
check('with no workdir either, it says so rather than inventing one', str_contains($tool4->describe('ls'), "container's default"), $tool4->describe('ls'));
