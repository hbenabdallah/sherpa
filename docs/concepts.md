...
- **API (the default).** Sherpa talks to any API that speaks the OpenAI
  ...
  Cloudflare Workers AI, vLLM, LM Studio, and gateways such as LiteLLM, which
  also reach Claude. You can record several providers and switch between them
  during a session. One provider's key is never sent to another.
