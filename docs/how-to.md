...
Sherpa speaks Anthropic's own API. You need an API key from the Anthropic
Console. A Claude Pro or Max subscription does not include API access.

```
❯ /provider add
  API address: https://api.anthropic.com/v1
  Name [anthropic]:
  API key (hidden; Enter if it needs none):
```

Then choose a model from the list, for example `claude-sonnet-5` or
`claude-opus-5`.

What Sherpa does for you:

- **Prompt caching.** Every request resends the whole conversation. Sherpa
  marks the system prompt and the conversation so that Anthropic serves the
  repeated part from its cache, which costs a fraction of the normal input
  price. `/context` shows how many tokens came from the cache.
- **Thinking.** Each model thinks according to its own default. The model's
  reasoning is sent back with the conversation, as the API expects. When Sherpa
  compacts the history, it drops the older reasoning, which no longer matches
  the rewritten history.

Anthropic has no embedding model, so documentation search uses keywords only,
and Sherpa says so.

A gateway that speaks the OpenAI dialect, such as OpenRouter or LiteLLM, also
reaches Claude. Add it like any other provider. Sherpa then speaks the OpenAI
dialect to it, without Anthropic's prompt caching.
