...
(`make bench`) or on real questions (`tests/rag/`). The later benchmark passes,
under [A provider under load](#a-provider-under-load), used
`nemotron-3-super-120b-a12b` on NVIDIA's API. The samples are small, and one
task can take 27 requests on one run and 34 on the next. Treat them as evidence
for a direction, not as benchmarks to compare models.
...
## A provider under load

On NVIDIA's free tier, the first benchmark pass lost 8 of 20 sessions to "two
empty replies in a row". Replaying the same requests gave proper tool calls, and
Sherpa read the recorded stream correctly, so the fault was not in parsing.

The cause showed up once Sherpa stopped ignoring one thing: an error sent
*inside* the stream (`data: {"error": ...}`), after the reply had started. It
used to be read as an empty reply, and the model took the blame. Raised, it
said "Service temporarily overloaded", which was behind all 11 failures of the
next pass. Every session that the provider did not interrupt passed.

Sherpa now asks again when the error names a passing condition (overload, rate
limit, timeout, unavailable), with the same patience as for an HTTP 503: up to
three attempts, 2 s then 4 s apart. It only does so while nothing has reached
the screen, since a second reply would be printed after the first half.

| Pass | Passed | Sessions lost to the provider |
|---|---|---|
| empty replies taken at face value | 10/20 | 8 |
| errors in the stream raised | 9/20 | 11 |
| **and retried** | **16/20** | **0** |

The four failures left were the model's own: code written without the test the
request asked for (three times), and a class renamed but not its file.
...
| A prompt rule: "add the test the request asks for before concluding" | The three scenarios behind the missing tests, run again without the rule, passed 8 of 9: the earlier failures were run-to-run variance, and there was no gap left for a rule to close. Not added. |
