...
PHP, ni Docker, rien d'autre à installer. Prenez la dernière version sur la page
[Releases](../../releases) :
...
`~/.local/bin` doit être dans votre `PATH`. Sur macOS, un fichier téléchargé
avec un navigateur plutôt qu'avec `curl` est mis en quarantaine et refusé au
lancement : `xattr -d com.apple.quarantine ~/.local/bin/sherpa` la lève.
