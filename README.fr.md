...
```sh
export SHERPA_API_KEY=…      # la clé de votre fournisseur ; jamais écrite sur disque
cd ~/code/mon-projet
sherpa
```
...
Options : `-b api|ollama` (backend pour cette exécution), `-m <modèle>`,
...
| Où | Quoi |
|---|---|
| `~/.config/sherpa/config.yaml` | backend, adresse du fournisseur, modèles choisis, tarifs (pour afficher un coût) |
...
| Variable | |
|---|---|
| `SHERPA_API_KEY` | la clé du fournisseur |
